...
class ColorLineAPI {
    constructor() {
        ...
        // If the API stops working, these need to be updated by inspecting
        // the network requests on https://colorline.my.site.com/CC/s/
        ...
        try {
            ...
            try {
                ...
                if (data.actions && data.actions[0] && data.actions[0].state === 'ERROR') {
                    console.error('Aura API Error:', data.actions[0].error);
                    throw new Error('Aura API returned an error');
                }
